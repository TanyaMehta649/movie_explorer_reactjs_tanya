...
import { useParams, useNavigate } from 'react-router-dom'; // ✅ add useNavigate
import axios from 'axios';
import toast from 'react-hot-toast'; // ✅ show toast if restricted
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      try {
        ...
        const token = parsed?.token; // Fetch token for authenticated requests
        ...
        // 🔐 Restriction logic: Only allow premium if user has premium plan or is supervisor
        if (movieData.premium) {
          const userPlan = parsed?.plan;
          const userRole = parsed?.role;

          if (userRole !== 'supervisor' && userPlan !== 'premium') {
            toast.error('Please subscribe to a premium plan to access this movie!');
            navigate('/dashboard');
            return;
          }
        }

        setMovie(movieData); // Set movie data after the restriction check
      } catch (err) {
        console.error('Error fetching movie:', err);
      }
    };

    fetchMovie(); // Call fetchMovie on page load
  }, [id, navigate]); // Effect depends on the movie id and navigate

  // Check if the movie data is still loading
  if (!movie) return <div className="text-white p-4">Loading...</div>;
